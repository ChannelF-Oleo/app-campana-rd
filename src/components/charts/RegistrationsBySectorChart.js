...
// Diseñada para que sectores adyacentes tengan alto contraste
const SECTOR_COLORS = [
...
function RegistrationsBySectorChart({ filterUserIds }) {
    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        ...
        snapshot.docs.forEach((doc) => {
          const data = doc.data();
          const sector = data.sector || "Por Asignar";
          countsBySector[sector] = (countsBySector[sector] || 0) + 1;
        });

        const labels = Object.keys(countsBySector).sort(); // Orden alfabético
        const data = labels.map((label) => countsBySector[label]);
        ...
        setChartData({
          ...
          datasets: [
            {
              label: filterUserIds ? "Tu Equipo" : "Total por Sector",
              data: data,
      ...
      (error) => {
        console.error("Error cargando gráfica de sectores:", error);
        setLoading(false);
      }
    );
...
export default RegistrationsBySectorChart;
