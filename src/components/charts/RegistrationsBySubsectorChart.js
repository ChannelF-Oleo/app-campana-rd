...
// Diseñada para que subsectores adyacentes tengan alto contraste
...
// NOTA: hoy solo ZONA N tiene sector/subsector poblado, así que la mayoría de
// los registros caerán en "Por Asignar" hasta que existan registros de Hato
// Nuevo. Es correcto, no un bug.
function RegistrationsBySubsectorChart({ filterUserIds }) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(
      q,
      (snapshot) => {
        const countsBySubsector = {};

        snapshot.docs.forEach((doc) => {
          const data = doc.data();
          const subsector = data.subsector || "Por Asignar";
          countsBySubsector[subsector] = (countsBySubsector[subsector] || 0) + 1;
        });

        const labels = Object.keys(countsBySubsector).sort(); // Orden alfabético
        const data = labels.map((label) => countsBySubsector[label]);

        // Asignación cíclica de colores
        const backgroundColors = labels.map(
          (_, index) => SUBSECTOR_COLORS[index % SUBSECTOR_COLORS.length]
        );
        ...
        setChartData({
          ...
          datasets: [
            {
              label: filterUserIds ? "Tu Equipo" : "Total por Subsector",
              data: data,
      ...
      (error) => {
        console.error("Error cargando gráfica de subsectores:", error);
        setLoading(false);
      }
    );
...
export default RegistrationsBySubsectorChart;
