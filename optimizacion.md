...
- [ ] Eliminar el `ProtectedRoute` duplicado (inline en `App.js` vs `src/ProtectedRoute.js` muerto)
- [ ] Extraer el bloque "welcome-row" repetido 3× en `Dashboard.js` a `<DashboardWelcome/>`
...
- [ ] Mover `useMediaQuery` de `App.js` a `hooks/useMediaQuery.js`

### 🗃️ Manejo de estado
- [ ] Introducir cache de datos (React Query/SWR) para evitar lecturas Firestore redundantes
- [ ] Importar constantes desde `constants.js` en `RegisterByActivist.js` (no redefinir `PROVINCIA_FIJA`, `validarCedula`, etc.)
- [ ] Reemplazar literales de rol mágicos (`"admin"`, `"lider de zona"`) por `ROL_ADMIN`/`ROL_LIDER`

### ⚡ Rendimiento
- [ ] Aplicar `React.lazy` + `Suspense` por ruta (mapas, admin, gráficos)
...
### 🧪 Pruebas
- [ ] Arreglar/eliminar `App.test.js` obsoleto ("learn react link")
...
- [ ] Quitar `console.log("DEBUG API Key…")` y comentarios de diagnóstico en `firebase.js`
- [ ] Confirmar que `.env` no está versionado; rotar claves si se filtró
- [ ] Eliminar `initializeAuthAndGetUser` si es código muerto
...
|---|-----------|-------|-----------|--------|
| 1 | 🔴 Alta | Quitar `console.log` de API key y diagnóstico en `firebase.js` | Documentación | ⬜ Pendiente |
| 2 | 🔴 Alta | Confirmar `.env` no versionado; rotar claves si se filtró | Documentación | ⬜ Pendiente |
| 3 | 🔴 Alta | Arreglar/eliminar `App.test.js` obsoleto | Pruebas | ⬜ Pendiente |
| 4 | 🔴 Alta | Eliminar `ProtectedRoute.js` duplicado y `initializeAuthAndGetUser` muertos | Estructura | ⬜ Pendiente |
| 5 | 🔴 Alta | Añadir Error Boundary global | UX | ⬜ Pendiente |
| 6 | 🟡 Media | Importar constantes desde `constants.js` (roles, ubicación, regex) | Estado | ⬜ Pendiente |
| 7 | 🟡 Media | Extraer `<DashboardWelcome/>` y `useMediaQuery` a sus archivos | Estructura | ⬜ Pendiente |
| 8 | 🟡 Media | Aplicar `React.lazy`/`Suspense` por ruta | Rendimiento | ⬜ Pendiente |
